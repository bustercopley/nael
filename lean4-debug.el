...
The buffer is given by BUFFER-NAME.  The message is given by
...
The message is given by FORMAT-STRING and ARGS."
  (when lean4-debug-mode
    (let ((time-str (format-time-string "%T.%3N" (current-time))))
      (lean4-output-to-buffer
          lean4-debug-buffer-name (concat "%s -- " format-string "\n")
        (cons
         (propertize time-str 'face 'font-lock-keyword-face) args)))))
