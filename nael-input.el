;;; nael-input.el --- The Lean input method (based/copied from Agda) -*- lexical-binding: t; -*-
...
;;
;; Use M-x customize-group nael-input to customise this input method.
...
;;
;; Use nael-input-show-translations to see all the characters which
...
;; performed until `nael-input-setup' is called at the end of this
;; file.

(defgroup nael-input nil
  "The Lean input method.
After tweaking these settings you may want to inspect the resulting
translations using `nael-input-show-translations'."
  :group 'nael)

(defcustom nael-input-inherit
  `(("TeX" . (nael-input-compose
              (nael-input-drop '("geq" "leq" "bullet" "qed" "par"))
              (nael-input-or
               (nael-input-drop-prefix "\\")
               (nael-input-or
                (nael-input-compose
                 (nael-input-drop '("^o"))
                 (nael-input-prefix "^"))
...
the argument of `nael-input-add-translations'.

The inherited translation pairs are added last, after
`nael-input-user-translations' and `nael-input-translations'.

If you change this setting manually (without using the
customization buffer) you need to call `nael-input-setup' in
order for the change to take effect."
  :group 'nael-input
  :set 'nael-input-incorporate-changed-setting
  :initialize 'custom-initialize-default
  :type '(repeat (cons (string :tag "Quail package")
                       (sexp :tag "Tweaking function"))))

(defcustom nael-input-user-translations nil
...
(defun nael-input-get-translations (qp)
...
(defun nael-input-show-translations (qp)
...
(Except for those corresponding to ASCII)."
  (interactive (list (read-input-method-name
                      "Quail input method (default %s): " "Nael")))
  (let ((buf (concat "*" qp " input method translations*")))
    (with-output-to-temp-buffer buf
      (with-current-buffer buf
        (quail-insert-decode-map
         (cons 'decode-map (nael-input-get-translations qp)))))))

(defun nael-input-add-translations (trans)
...
(defun nael-input-inherit-package (qp &optional fun)
...
a list of such pairs."
  (let ((trans (nael-input-get-translations qp)))
    (nael-input-add-translations
     (if fun (nael-input-concat-map fun trans)
       trans))))
...
well."
  ...
  (nael-input-add-translations
   (mapcar (lambda (tr) (cons (car tr) (vconcat (cdr tr))))
           nael-input-user-translations))
  (nael-input-add-translations nael-input-translations)
  (dolist (def nael-input-inherit)
    (nael-input-inherit-package (car def)
                                 (eval (cdr def)))))

(defun nael-input-incorporate-changed-setting (sym val)
  "Update the Lean input method.
Set SYM default value to VAL, then call `nael-input-setup'.
      ...
      (dolist
          (translation
           (seq-filter
            ...
            (nael-input-get-translations "Nael")))
...
(defun nael-input-export-translations-to-stdout ()
  "Print current translations to stdout."
  (nael-input-export-translations)
  (with-current-buffer "*nael-translations*"
    (princ (buffer-string))))

(provide 'nael-input)

;;; nael-input.el ends here
