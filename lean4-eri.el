...
;; Adapted from agda-mode, accessible at
;; https://github.com/agda/agda/blob/master/src/data/emacs-mode/eri.el

;;; Code:

(require 'cl-lib)

(defun lean4-eri-current-line-length ()
  "Calculate length of current line."
  (- (line-end-position) (line-beginning-position)))

(defun lean4-eri-current-line-empty ()
  "Return non-nil if current line empty (not counting white space)."
  (equal (current-indentation) (lean4-eri-current-line-length)))
...
XS1 are <= X, whereas all elements in XS2 are > X."
  (let*
      ((pos (or (cl-position-if (lambda (y) (> y x)) xs) (length xs)))
...
the returned list."
  ...
  (let ((points) (max) (start (line-beginning-position)))
    (save-excursion
      (while
          (progn
            (forward-line -1)
    ...
    ;; (Note that sort and nreverse are destructive.)
    (let*
        ((ps0
          (remove
           (current-indentation)
...
follows:
  ...
  * A new indentation point is also added, two steps in from the
  ...
  (interactive "P")
  (let*
      ((points (lean4-eri-calculate-indentation-points reverse))
       (remaining-points (cdr (member (current-indentation) points)))
       (indentation
        (if remaining-points (car remaining-points) (car points))))
    (when indentation
